    ...
    def handle(self, *args, **options):
        ...
        with open(filename, 'r') as f:
            ...
            for item in data:
                ...
                program_data = {
                    'subject': data[str(item)]['subject'],
                    'subjectDescription': data[str(item)]['subjectDescription']
                }
                program_instance = Program.objects.get_or_create(**program_data)[0]

                # Create Faculty instances
                valid_faculty_data = []
                for faculty in data[str(item)]['faculty']:
                ...
                meeting_time_data = data[str(item)]['meetingsFaculty']
                ...
                meeting_location_data = data[str(item)]['meetingsFaculty']
                ...
                valid_course_data = {field: data[str(item)][field] for field in course_fields}
                ...
                valid_section_data = {field: data[str(item)][field] for field in section_fields}
                ...
                for i, faculty in enumerate(faculty_instances):
                    valid_instructor_role_data = {
                        ...
                        'primaryInstructor': data[str(item)]['faculty'][i]['primaryIndicator']
                    }
